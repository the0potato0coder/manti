(function() {
  ...
  const isAllowedHost = host.endsWith('orchids.cloud');
  ...
  if (!projectId) {
    try {
      ...
      const match = name.match(/orchids:projectId=([a-zA-Z0-9_-]+)/);
      if (match && match[1]) {
        projectId = match[1];
      } else if (name && name.charAt(0) === '{') {
        const parsed = JSON.parse(name);
        if (parsed && parsed.orchidsProjectId) {
          projectId = String(parsed.orchidsProjectId);
        }
      }
    } catch {}
  }
